...
class AllocationPage extends BasePage {
  ...
  constructor(page) {
    ...
    //
    // campaignTypeSelect: label-relative — UAT form order changed (Project now
    // renders before Allocation Type so #rc_select_0 is Project, not Type).
    // Using a stable label-based locator avoids drift when field order changes.
    ...
    this.campaignTypeSelect     = page.locator('.ant-form-item').filter({ hasText: /Allocation Type/ }).locator('.ant-select').first();
    this.projectSelect          = page.locator(L['rcSelect0']              && L['rcSelect0'].selector);
