 ...
 *
 * Run:
 *   node src/test/agents/page-doc-agent.js
 * ============================================================
 */
...
\`src/test/automation/login.spec.js\`

## Execution Commands

\`\`\`bash
# Run all login tests
npx playwright test --config resources/config/playwright.config.js login.spec.js --headed --workers=1

# Run only positive
npx playwright test --config resources/config/playwright.config.js login.spec.js -g "POSITIVE" --headed --workers=1

# Run only negative
npx playwright test --config resources/config/playwright.config.js login.spec.js -g "NEGATIVE" --headed --workers=1
\`\`\`
...
## Page Object File
\`src/main/js/pages/customers.page.js\`

## Automation Test File
\`src/test/automation/customers.spec.js\`
`,
};
...
function generatePageDoc(moduleName, content) {
...
async function run() {
  ...
  if (fs.existsSync(PORTAL_MAP_FILE)) {
    const portalMap = JSON.parse(fs.readFileSync(PORTAL_MAP_FILE, 'utf-8'));
