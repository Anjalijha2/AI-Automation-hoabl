...
| TC_LEADS_FUNC_007 | "Resend Notification" action triggers re-engagement backend | High | A lead row exists with valid phone/email | 1. Locate first lead row's Action cell<br>2. Click `button.reset-btn` whose SVG `<title>` = "Resend Notification" (paper-airplane icon)<br>3. Verify backend API call fires (per BRD §1.6) | Resend API is fired and lead receives re-engagement notification. No toast confirmation is expected — silent success is intended UX. | `leads-share-action.png` | CP-FS-Leads §1.6 (System Actions — re-engagement) | Approved |
| TC_LEADS_FUNC_008 | "Copy Link" action copies referral URL to clipboard | High | Browser context has clipboard-write permission; one lead row visible | 1. Locate first lead row's Action cell<br>2. Click `button.reset-btn` whose SVG `<title>` = "Copy Link" (document/copy icon)<br>3. Read system clipboard contents | Clipboard contains a referral URL of form `https://uat.xrportal.in/ref/<sha256-like-token>` (e.g. `https://uat.xrportal.in/ref/02e2e02ca41382306396dbb87cde0bfd5e5c6d6e8b5dc6c89f93f8c4456e0929`). Copy succeeds silently — no toast required. | `leads-copy-action.png` | CP-FS-Leads §1.6 (CP shares lead invitation) | Approved |
...
None. Both action buttons function correctly as confirmed by user (2026-06-07):
- "Resend Notification" — backend API fires, lead receives the resend.
- "Copy Link" — referral URL is correctly copied to clipboard.

The absence of a confirmation toast is intentional / accepted UX — not a defect.

**Reviewer correction:** Earlier review batch incorrectly flagged UX-LEADS-001 and UX-LEADS-002 as bugs. Removed 2026-06-07.
