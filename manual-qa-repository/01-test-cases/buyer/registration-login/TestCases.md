...
**Status:** Conditional (QA test-case-reviewer pass 2026-06-06 — 13 Approved + 1 Conditional; see review-report.md)
...
| TC_BUYER_LOGIN_NEG_005 | FRD 1.5 | buyer | Registration & Login | NEG | Wrong OTP rejected — user stays on login page, no JWT issued | Unauthenticated session; OTP boxes visible | 1. Navigate to `https://uat.xrportal.in/`<br>2. Enter mobile `8888888888`<br>3. Click "Send OTP"<br>4. Enter incorrect OTP `000000` across the six boxes<br>5. Click "Verify"<br>6. Inspect URL and sessionStorage | URL remains `https://uat.xrportal.in/` (no redirect to `/home`); sessionStorage `xr_auth_token` is NOT set; an error indication (toast/inline) is shown to user (specific UI to be validated post-screenshot capture) | `[NO-VISUAL-EVIDENCE]` | mobile=`8888888888`, otp=`000000` | P1 | Conditional |
