 ...
 *
 * Run:
 *   npx ts-node ai-agents/defect-agent.ts
 * ============================================================
 */
...
function severityFromTitle(title: string): Bug['severity'] {
 ...
 */
function readExistingBugs(): { cards: string; existingTcIds: Set<string>; bugCount: number } {
...
function formatBugCard(bug: Bug): string {
  let card = `## ${bug.bugId}\n\n`;
  card += `| Field              | Value |\n`;
  card += `|--------------------|-------|\n`;
  card += `| **TC_ID**          | ${bug.tcId} |\n`;
...
function run(): void {
  ...
  if (!fs.existsSync(RESULTS_FILE)) {
    ...
    console.log('   ℹ️  Run tests first: npm run test:regression  OR  npm run execute');
    return;
  }
