 ...
 *
 * Run:
 *   npx ts-node ai-agents/sprint-manager.ts status     → show coverage
 *   npx ts-node ai-agents/sprint-manager.ts update     → update all docs
 *   npx ts-node ai-agents/sprint-manager.ts plan-brd   → generate sprint plan from brd/*.md
 * ============================================================
 */
...
const MODULE_COVERAGE: ModuleCoverage[] = [
  { feature: 'Login',            totalTC: 18, automated: 18, specFile: 'login.spec.ts' },
  { feature: 'Customers',        totalTC: 6,  automated: 6,  specFile: 'customers.spec.ts' },
...
function generateSprintLog(): string {
  const timestamp = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' });
  return `# Sprint Log
...
- [x] Configured \`playwright.config.ts\` with projects (auth-setup, login-tests, smoke, regression)
- [x] Created Auth Setup (\`auth.setup.ts\`) for session caching
- [x] Implemented Login Page Object (\`login.page.ts\`)
- [x] Implemented 18 Login test cases (Positive, Negative, Functional, Security)
- [x] Implemented Customers Page Object (\`customers.page.ts\`)
- [x] Implemented 6 Customers test cases
- [x] Created all 8 AI Agent scripts (\`ai-agents/\` folder)
...
function generateTaskTracker(): string {
  return `# Task Tracker
 ...
 */
function appendChangelog(newEntry: string): void {
...
function buildChangelogEntry(): string {
...
function parseBRD(content: string): Epic[] {
...
function generateSprintPlanDoc(epics: Epic[], brdFileName: string): string {
  const timestamp = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' });
