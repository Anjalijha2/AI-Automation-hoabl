...
async function getRegistrationCounts(page) {
...
function buildXlsx(filename, rows, includeHeader = true) {
    const dir = path.join(process.cwd(), 'src', 'test', 'test-data');
...
// Visually open an Excel file using the Windows shell
function visuallyOpenExcel(filePath) {
    try {
        console.log(`Physically opening Excel file on screen: ${filePath}`);
        execSync(`start "" "${filePath}"`);
    } catch (e) {
        console.error("Could not open Excel on the desktop:", e);
    }
}

// The file input for the Registration Status section upload
async function getRegistrationUploadInput(page) {
...
async function getRegistrationSubmitButton(page) {
...
test.describe('Registration Status — TC-2.1 to TC-2.7', () => {
    ...
    test('TC-2.1: Forbid registration → inactive count increases', async ({ page }) => {
        ...
        // 👁️ VISUAL VERIFICATION FOR USER: Physically open the file that is about to be uploaded
        visuallyOpenExcel(filePath);
        await page.waitForTimeout(5000); // Give user 5 seconds to look at the newly created Excel file
        ...
        if (download) {
            const savePath = path.join(process.cwd(), 'src', 'test', 'test-data', 'tc2_1_final_output.xlsx');
            ...
            // 👁️ VISUAL VERIFICATION FOR USER: Physically open the downloaded "Final Excel" file
            visuallyOpenExcel(savePath);
            await page.waitForTimeout(5000); // Give user 5 seconds to look at the final downloaded status in Excel
    ...
    test('TC-2.2: Allow registration → active count updates', async ({ page }) => {
        ...
        if (download) {
            const savePath = path.join(process.cwd(), 'src', 'test', 'test-data', 'tc2_2_final_output.xlsx');
            await download.saveAs(savePath);
            const wb = XLSX.readFile(savePath);
            const data = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { header: 1 });
            ...
            // 👁️ VISUAL VERIFICATION FOR USER: Physically open the downloaded "Final Excel" file
            visuallyOpenExcel(savePath);
            await page.waitForTimeout(5000); // Give user 5 seconds to view the file
    ...
    test('TC-2.3: Sample file download → correct structure', async ({ page }) => {
        ...
        if (download) {
            const savePath = path.join(process.cwd(), 'src', 'test', 'test-data', 'sample_reg_status.xlsx');
            await download.saveAs(savePath);
